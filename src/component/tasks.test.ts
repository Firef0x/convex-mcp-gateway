...
describe("tasks: completion and failure", () => {
  ...
  test("a deep result whose envelope blows past the backstop is rejected", async () => {
    const t = newTest();
    await t.run(async (ctx) => {
      await createTask(ctx);
      // Pretty-printing indents per nesting level, so an envelope can be
      // far more than 3x its value. Rejecting it here is what keeps the
      // oversize case a clean `result_too_large` instead of a Convex
      // argument-limit rejection that leaves the row stuck `working`.
      let deep: unknown = "x".repeat(1024);
      for (let i = 0; i < 90; i++) deep = [deep];
      const padded = { blob: "y".repeat(3 * 256 * 1024), deep };
      expect(
        await ctx.runMutation(api.tasks.completeTask, {
          taskId: "task-1",
          result: padded,
      ...
      expect(task?.status).toBe("failed");
      expect(task?.error?.message).toMatch(/envelope exceeds/);
    });
  });
  ...
  test("an oversized result fails the task instead of storing it", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      expect(
        await ctx.runMutation(api.tasks.completeTask, {
          taskId: "task-1",
          // Past the STORAGE backstop, which is deliberately larger than
          // the documented data cap because the CallToolResult envelope
          // carries the value twice (escaped text plus structuredContent).
          result: { blob: "x".repeat(3 * 256 * 1024 + 1) },
        }),
