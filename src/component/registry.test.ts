...
describe("registry", () => {
  ...
  test("setAuthorizer + setOAuthConfig round-trip and preserve each other", async () => {
    const t = convexTest(schema, modules);
    await t.run(async (ctx) => {
      await ctx.runMutation(api.registry.setAuthorizer, {
        authorizerHandle: "fake-authorizer",
      });
      await ctx.runMutation(api.registry.setOAuthConfig, {
        authServerUrl: "https://idp.example.com/",
        resourceUrl: "https://app.example.com/mcp/",
      ...
      // Both still readable; neither wiped the other.
      expect(
        await ctx.runQuery(api.registry.getAuthorizer, {}),
      ).toBe("fake-authorizer");
      expect(
        await ctx.runQuery(api.registry.getOAuthConfig, {}),
      ).toEqual({
        authServerUrl: "https://idp.example.com/",
      ...
      // Clearing the auth server clears resourceUrl too (it would be
      // dangling); the authorizer is untouched.
      await ctx.runMutation(api.registry.setOAuthConfig, {
        authServerUrl: null,
      });
      expect(
