...
/**
 * Cap on a tool's own result value, which is what `docs/tasks.md`
 * promises and what the built-in executor measures before wrapping.
 */
export const TASK_MAX_RESULT_BYTES = 256 * 1024;
/**
 * Storage backstop on the stored `CallToolResult`. The envelope carries
 * the value TWICE for a tool with an `outputSchema` (JSON-escaped inside
 * `content[0].text`, and again as `structuredContent`), and escaping can
 * double the text copy, so a legal 256 KiB result usually serializes to
 * about 3x that. Measuring the envelope against the data cap would
 * silently cut the documented limit to a third of itself, and
 * unpredictably, since the factor depends on escaping and on nesting
 * depth (pretty-printing indents per level, so a deep value can exceed
 * 3x by a lot). That is exactly why this is a backstop and not a
 * conversion factor: the executor checks it after wrapping. Still well
 * inside Convex's 1 MiB document limit.
 */
export const TASK_MAX_STORED_RESULT_BYTES = 3 * TASK_MAX_RESULT_BYTES;
...
export const completeTask = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (exceeds(args.result, TASK_MAX_STORED_RESULT_BYTES)) {
      const error = {
        code: -32000,
        message: `Stored task result envelope exceeds ${TASK_MAX_STORED_RESULT_BYTES} serialized bytes`,
      };
      await ctx.db.patch("tasks", row._id, {
        status: "failed",
        error,
    ...
    // A completed call whose result says `isError` is a business failure,
    // and it must not read as a clean success in the audit log: for a
    ...
    const resultIsError =
      isPlainRecord(args.result) && args.result.isError === true;
    await recordTaskAudit(
      ctx,
      row,
      "complete",
...
export const executeScheduledTask = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!dispatched.ok) {
      // Same split as the synchronous path (MCP 2025-06-18 §tools/call):
      //   - a PROTOCOL error (unknown tool / invalid args, -32602) aborts
      //     the call, so the task genuinely failed;
      //   - a TOOL EXECUTION error is a *successful* call whose result
      //     says `isError: true`, which is what lets the model reason
      //     about it and retry.
      // Reporting the second as a failed task would give a task-run tool
      // a different contract from the same tool called synchronously, and
      // would hide a business error behind a transport-shaped failure.
      ...
      // Enumerate what may be laundered into a result rather than what
      // may not: `runTool` returns -32000 for anything the tool itself
      // threw (including its arg-validator), -32602 for an unknown tool,
      // and -32001 when a tool that needs a caller has none. Only the
      // first is a call that ran. The other two are refusals, and a
      // refusal reported as `completed` with prose about being
      // unauthorized is the worst shape available here; a code added
      // later defaults to failing the task rather than to lying.
      ...
      await finalize(
        ...
        args.taskId,
        task.toolName,
        callToolResult(dispatched.error.message, undefined, true),
        true,
      );
      return null;
    }
    // The tool's own value is what the documented 256 KiB cap applies to,
    // so check it BEFORE wrapping: `completeTask`'s guard sees the envelope
    // (value twice over, escaped), and measuring that against the data cap
    // would cut the documented limit to a third of itself.
    if (exceeds(dispatched.data, TASK_MAX_RESULT_BYTES)) {
      console.error(
        "[mcp-gateway] result value over cap: the tool succeeded but its " +
          "value exceeds the documented result size; the task is failed " +
    ...
    // The stored result IS the `CallToolResult` the synchronous path would
    // have returned, wrapped here rather than at the polling boundary so
    // `tasks/get` needs no tool metadata and every executor stores one
    // shape. A client renders `content`, validates `structuredContent`
    // against the advertised `outputSchema`, and reads `isError` exactly
    // as it would from `tools/call`.
    const result = callToolResult(
      dispatched.data,
      tool.outputSchema !== undefined,
      false,
    );
    // The envelope, not just the value: `JSON.stringify(v, null, 2)` adds
    // indentation per nesting level, so a deep-but-legal result can inflate
    // far past the 3x rule of thumb. Left unchecked it would be rejected by
    // Convex at the `runMutation` boundary instead of by `completeTask`,
    // landing in `finalize`'s catch and stranding the row as `working`
    // after the tool's side effects committed. Fail it cleanly here.
    if (exceeds(result, TASK_MAX_STORED_RESULT_BYTES)) {
      console.error(
        "[mcp-gateway] stored envelope over backstop: the tool's value fit " +
          "the result cap but its CallToolResult does not; the task is " +
          "failed and the result discarded",
        args.taskId,
        task.toolName,
      );
      await fail({
        code: -32000,
        message: `Task result envelope exceeds ${TASK_MAX_STORED_RESULT_BYTES} serialized bytes`,
      });
      return null;
    }
    await finalize(ctx, args.taskId, task.toolName, result, false);
    return null;
  },
});

/**
 * The MCP `CallToolResult` envelope, byte-identical in shape to what the
 * host handler builds for a synchronous `tools/call`: the text-JSON
 * `content` block every client understands, plus `structuredContent` when
 * the tool advertised an `outputSchema` (which spec-compliant clients
 * prefer), plus `isError`.
 *
 * For an error, `value` is the already-sanitized wire message and the
 * content block carries it as text, matching the synchronous path.
 */
function callToolResult(
  value: unknown,
  withStructured: boolean | undefined,
  isError: boolean,
  ...
  return {
    ...
    ...(withStructured === true ? { structuredContent: value ?? null } : {}),
    isError,
  };
}
...
async function finalize(
  ...
  /**
   * Whether `result` reports a tool error. Only affects wording, but the
   * wording is the whole value of these logs: telling an operator that
   * "the tool's committed side effects are not rolled back" when the tool
   * actually threw sends them hunting for writes that never happened.
   */
  isError: boolean,
): Promise<void> {
