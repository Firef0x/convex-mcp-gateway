...
- `tasks/get` (`params.taskId`, mirrored in `Mcp-Name`) polls the task.
  ...
  `isError: true`, mirroring the synchronous split in MCP 2025-06-18
  §tools/call: the model can read the message and retry. `failed` is
  reserved for the task itself never producing a result at all: an unknown
  tool, a tool whose kind changed since creation, a dispatch that never
  ran, an executor that could not start, a result too large to store.
  Reporting a business error as `failed` would give a task-run tool a
  different contract from the same tool called inline.
...
in `onInputResponses` (completing with a `CallToolResult`, and with
`isError` when the owner declines), sets its own `tasks.scope` so its
...
- `gateway.completeTask(ctx, taskId, result)`. The built-in executor
  stores a `CallToolResult` (see the wire contract above); a host executor
  owns the shape it stores, so pass the same envelope
  (`{ content: [...], structuredContent?, isError }`) if you want clients
  to treat your task results like ordinary tool results.
- `gateway.failTask(ctx, taskId, { code, message }, auditErrorMessage?)`.
  For the task *itself* failing. A tool that ran and produced a business
  error (a declined confirmation, a validation failure) should
  `completeTask` with `isError: true` instead, which is what the example
  does: `failed` tells a client the call never happened, and it cannot act
  on a message it is told to disregard.
...
- Serialized sizes: task arguments ≤ 64 KiB, `inputRequests` /
  ...
  to the tool's own value, measured before the `CallToolResult` envelope is
  built: the envelope carries that value twice (JSON-escaped inside
  `content[0].text`, and again as `structuredContent`), so the storage
  backstop on the stored row is three times larger. A host calling
  `completeTask` itself is subject to the backstop only.
