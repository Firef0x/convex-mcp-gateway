...
describe("MCP tasks (modern, e2e)", () => {
  ...
  test("task-augmented call defers execution and completes on poll", async () => {
    ...
    expect(done.result?.task.result).toEqual({
      content: [{ type: "text", text: JSON.stringify({ total: 2 }, null, 2) }],
  ...
  test("the task result is byte-identical to the synchronous result", async () => {
    ...
    try {
      ...
      expect(viaTask).toEqual({
        content: sync.result.content,
        structuredContent: sync.result.structuredContent,
        isError: sync.result.isError,
      });
    } finally {
      vi.useRealTimers();
    }
...
describe("MCP tasks (host executor, e2e)", () => {
  ...
  test("input_required round-trip: confirm, execute once, complete", async () => {
    ...
    // host executor follows the same rule as the built-in one.
    expect(done?.result).toEqual({
      content: [{ type: "text", text: JSON.stringify({ updated: 2 }, null, 2) }],
      isError: false,
    });
  ...
  test("a declined confirmation completes with isError, without side effects", async () => {
    const t = newTest();
    await t.run(async (ctx) => {
      await ctx.db.insert("invoices", { status: "open", amount: 1 });
    });
    ...
    // A decline is a business outcome, not a task failure: the call
    // completed and its result says isError, exactly as the synchronous
    // MRTR `completeCall()` path reports a declined confirmation.
