    ...
    tasks: {
      ...
      onInputResponses: async (ctx, event) => {
        ...
        if (confirm?.action !== "accept") {
          // A declined confirmation is a business outcome, not a task
          // failure, so it completes with `isError` exactly as the
          // synchronous MRTR `completeCall()` path would.
          const declined = await gateway.completeTask(ctx, event.taskId, {
        ...
        // Store the same `CallToolResult` envelope the built-in executor
        // (and a synchronous tools/call) produces, so a client cannot tell
        // which executor ran the tool. No `structuredContent`: this tool
        // declares no `returns`, so it advertises no `outputSchema`, and
        // MCP says structured output should only accompany one. The
        // built-in executor keys the block on exactly that.
        const completed = await gateway.completeTask(ctx, event.taskId, {
